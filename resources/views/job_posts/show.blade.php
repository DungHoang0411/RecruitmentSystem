            ...
            <div class="card-body">
                <div class="row mb-3">
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9">
                            @if ($jobPost->status === 'expired')
                                <span class="badge bg-danger text-white px-2 py-1 rounded">Đã hết hạn</span>
                            @elseif($jobPost->status === 'published')
                                <span class="badge bg-success text-white px-2 py-1 rounded">Đang tuyển</span>
                            @else
                                <span
                                    class="badge bg-secondary text-white px-2 py-1 rounded">{{ ucfirst($jobPost->status) }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-9 text-primary fw-bold">
                            {{ $jobPost->company ? str_ireplace('company_', '', $jobPost->company->name) : 'Chưa cập nhật' }}
                        </div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-md-3"><strong>Danh mục:</strong></div>
                        <div class="col-md-9">
                            {{ $jobPost->category ? str_ireplace('category_', '', $jobPost->category->name) : 'Chưa phân loại' }}
                        </div>
                    </div>
                    <div class="row mb-3">
