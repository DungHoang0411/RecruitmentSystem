...
@section('content')
    ...
    <div class="hero-banner text-center">
        <div class="container">
            ...
            <ul class="nav nav-pills justify-content-center gap-2">
                <li class="nav-item">
                    <a class="nav-link active bg-white text-success fw-bold px-4" href="{{ route('job-posts.index') }}">Việc làm</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-white border border-light px-4" href="{{ route('categories.index') }}">Danh mục</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-white border border-light px-4" href="{{ route('companies.index') }}">Công ty</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('tags.*') ? 'active bg-white text-success fw-bold' : 'text-white border border-light' }} px-4" href="{{ route('tags.index') }}">Thẻ (Tags)</a>
                </li>
            </ul>
        </div>
    </div>
    ...
    <div class="container mb-5">
        <div class="filter-bar mb-5">
            <form action="{{ route('job-posts.index') }}" method="GET" slug="filterForm">
                <div class="row g-3">
                    <div class="col-lg-12 mb-2">
                        <div class="input-group input-group-lg border rounded">
                            <span class="input-group-text bg-white border-0 text-muted"><i class="bi bi-search"></i></span>
                            <input type="text" name="search" class="form-control border-0 shadow-none ps-0" placeholder="Nhập tên vị trí, kỹ năng hoặc công ty..." value="{{ request('search') }}">
                            <button type="submit" class="btn btn-brand px-4">Tìm kiếm</button>
                            <a href="{{ route('job-posts.index') }}" class="btn btn-light px-4 border-start text-muted" title="Xóa tất cả bộ lọc">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="destination_country" class="form-select tom-select-filter" onchange="this.form.submit()">
                            <option value="" {{ empty(request('destination_country')) ? 'selected' : '' }}>Tất cả</option>
                            @foreach ($countries as $value => $label)
                                <option value="{{ $value }}" {{ request('destination_country') == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="visa_type" class="form-select tom-select-filter" onchange="this.form.submit()">
                            ...
                            @foreach ($visa_types as $value => $label)
                                <option value="{{ $value }}" {{ request('visa_type') == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('exports.job-posts', request()->all()) }}" class="btn btn-success">
                ...
                <a href="{{ route('exports.history') }}" class="btn {{ request()->routeIs('exports.history') ? 'btn-primary' : 'btn-outline-primary' }}">
                    <i class="bi bi-clock-history me-1"></i> Lịch sử xuất Excel
                </a>
            </div>
        </div>
        ...
        <div class="row g-4">
                ...
                <div class="col-xl-6 col-lg-6 col-md-12">
                    <div class="job-card">
                        ...
                        <div class="job-card__footer">
                            <div>
                                <span class="badge @if ($item->status == 'published') bg-success @elseif($item->status == 'draft') bg-secondary @elseif($item->status == 'closed') bg-dark @else bg-danger @endif">
                                    {{ $statuses[$item->status] ?? ucfirst($item->status) }}
                                </span>
                                <small class="text-muted ms-2"><i class="bi bi-eye"></i> {{ $item->view_count }} lượt xem</small>
                            </div>
            ...
            @empty
                <div class="col-12 text-center py-5">
                    <img src="https://www.topcv.vn/v4/image/empty.svg" alt="Empty" style="width: 150px; opacity: 0.5;" class="mb-3">
